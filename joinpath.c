/*-------------------------------------------------------------------------
 * joinpath.c
 *    Routines to consider and generate various paths for joins. 
 * 
 * The main purpose of this files is having access to static functions of 
 * cost model. Aother purpose is tweaking functions behavior by replacing 
 * the cost estimation function with ours, like replacing initial_cost_nestloop
 * with pg_hint_initial_cost_nestloop.
 * 
...
static List *select_mergejoin_clauses(PlannerInfo *root, RelOptInfo *joinrel,
...
static void consider_parallel_nestloop(PlannerInfo *root, RelOptInfo *joinrel, RelOptInfo *outerrel,
									   ...
									   JoinPathExtraData *extra);
static void consider_parallel_mergejoin(PlannerInfo *root, RelOptInfo *joinrel, RelOptInfo *outerrel,
										RelOptInfo *innerrel, JoinType jointype, JoinPathExtraData *extra,
...
static Path *get_memoize_path(PlannerInfo *root, RelOptInfo *innerrel, RelOptInfo *outerrel,
							  ...
							  JoinPathExtraData *extra);
static bool paraminfo_get_equal_hashops(PlannerInfo *root, ParamPathInfo *param_info, RelOptInfo *outerrel,
										RelOptInfo *innerrel, List **param_exprs, List **operators,
										bool *binary_mode);
                   ...
                   bool is_partial)
{
     ...
     */
    pg_hint_initial_cost_mergejoin(root, &workspace, jointype, mergeclauses,
				  ...
				  JoinPathExtraData *extra)
{
	 ...
	 */
	pg_hint_initial_cost_nestloop(root, &workspace, jointype,
						  outer_path, inner_path, extra);

	int innerrel_relids = 0;
	int outerrel_relids = 0;
	FILE *file = fopen("/tmp/lql_log.txt", "a+");
	for(int i = 0; i < innerrel->relids->nwords; i++)
	{
		innerrel_relids += innerrel->relids->words[i];
	}
	for(int i = 0; i < outerrel->relids->nwords; i++)
	{
		outerrel_relids += outerrel->relids->words[i];
	}
	if(innerrel_relids == 10 && outerrel_relids == 1024)
		fprintf(file, "innerrel->relids: %d, outerrel_relids: %d\n, workspace startup_cost=%f, total_cost=%f\n", 
				innerrel_relids, outerrel_relids, workspace.startup_cost, workspace.total_cost);
	fclose(file);
				  ...
				  JoinPathExtraData *extra)
{
	 ...
	 */
	pg_hint_initial_cost_hashjoin(root, &workspace, jointype, hashclauses,
